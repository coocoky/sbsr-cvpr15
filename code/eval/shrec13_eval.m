function shrec13_eval(test_case)
...
evalrst_dir = 'SHREC13_Evaluation_Ours';
shrec13_evalrst_dir = 'SHREC13_Evaluation_Results';
...
switch testname
  case 'train',
    sketch_data = load(sprintf('%s/sketch_train.mat', data_dir));
  case 'test',
    sketch_data = load(sprintf('%s/sketch_test.mat', data_dir));
  case 'complete',
    sketch_data = load(sprintf('%s/sketch_train.mat', data_dir));
    sketch_test_data = load(sprintf('%s/sketch_test.mat', data_dir));
...
view_data = load(sprintf('%s/view_shrec.mat', data_dir));
...
[Pmat_shrec, Rmat_shrec, names_shrec, plotentries_shrec] = read_eval_results(shrec13_evalrst_dir, testname, 0);

alg_names = [names_ours, names_shrec];
plotentries = [plotentries_ours, plotentries_shrec];

colorset = jet(size(plotentries, 2)+2);
...
hl = legend(alg_names);
set(gca, 'Position', [0.08, 0.1, 0.53, 0.85], 'Fontsize', 12);
box on;
set(hl,'Interpreter','none', 'Fontsize', 9, 'Location', 'NorthEastOutside')
set(hl, 'Position', [0.66,0.1,.2,.9])

xlabel('Recall', 'Fontsize', 12);
ylabel('Precision', 'Fontsize', 12)
ylim([0,0.65]);
%   set(hf, 'Position', [2800 120  960 800]);
set(gcf, 'Units','centimeters', 'Position',[4 9 22.5 14]);
